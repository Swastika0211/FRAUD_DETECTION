...
def dark_layout(**kwargs):
    ...
    Merges xaxis/yaxis kwargs so callers can pass extra axis options
    without conflicting with the base gridcolor settings.
    """
    ...
    base = dict(
        ...
        xaxis=dict(gridcolor='#1a2a4a', zerolinecolor='#1a2a4a', **xaxis_extra),
        yaxis=dict(gridcolor='#1a2a4a', zerolinecolor='#1a2a4a', **yaxis_extra),
    )
    base.update(kwargs)
    return base
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if "models" not in st.session_state:
        ...
        with col2:
            ...
            fig_bar.update_layout(**dark_layout(yaxis=dict(range=[0.7,1.0])),height=340,barmode='group',
